...
@include('crud::fields.inc.wrapper_start')
    ...
    <div class="array-container form-group">
            ...
            <tbody class="table-striped items sortableOptions">
                ...
                <tr class="array-row clonable" style="display: none;">
                    ...
                    <td>
                        ...
                        @if (isset($field['column_types'][$column]))
                            @switch($field['column_types'][$column]['type'])
                                @case('select')
                                    <select 
                                        class="form-control form-control-sm" 
                                        type="text" 
                                        data-cell-name="item.{{ $column }}"
                                        {{ isset($field['column_types'][$column]['disabled']) ? 'disabled' : ''  }}
                                    >
                                        <option value="" selected disabled>-</option>
                                        @foreach ($field['column_types'][$column]['options'] as $id => $name)
                                ...
                                @case('text')
                                    <input 
                                        ...
                                        type="text"
                                        data-cell-name="item.{{ $column }}"
                                        {{ isset($field['column_types'][$column]['readonly']) ? 'readonly': '' }}
                                    >
                                    @break
                                @default
                            @endswitch
        ...
        <div class="array-controls btn-group m-t-10" style="display: none">
                ...
                if($rows != '[]') {
                    $.each($rows, function(key) {
                        ...
                        $.each(this, function(column , value) {
                            ...
                            $tableWrapper.find('tbody tr:last').find('select[data-cell-name="item.' + column + '"]').val(value);
                        });
